...
mkPolicy pkh deadline () ctx =  traceIfFalse "invalid public key hash" isPKH &&
                                traceIfFalse "deadline has been reached" afterDeadline
    where
        info :: TxInfo
        info = scriptContextTxInfo ctx

        isPKH :: Bool
        isPKH = txSignedBy info pkh

        afterDeadline :: Bool
        afterDeadline = contains (to $ deadline) $ txInfoValidRange info
...
mint mp = do
    ...
    if now > deadline then
		Contract.logError @String "deadline passed"
	else do
