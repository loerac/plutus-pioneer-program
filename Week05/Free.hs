...
{-
 ...
 - `Integer`. This is added in our `FreeSchema` to only accept a data type of
 - that matter.
 -
...
{-
 - @brief:  Most basic `MonetaryPolicy`, similar to the `mkValidator`. Only
 -          receive `ScriptContext` and ignore the input and returns true.
 - @param:  ScriptContext - Only paramater used since minting only uses
 -          transaction
...
{-
 ...
 -
 - The first thing done is create a `Value` using the `Value.singleton`
 - function. The `Value` is stored in `val` which is a map of the `curSymbol`
 - of the policy, `mpTokenName` (from `MintParams`), and `mpAmount` (from
 - `MintParams`).
 -
 - Secondly, given the policy, look up the policy to create a transaction. This
 - is used to add additional information to the transaction validation.
 -
 - Thirdly, create a transaction, `tx`, with the `Constraints.mustForgeValue`
 ...
 -
 - Forthly, wait (`awaitTxConfirmed`) for the transaction (`ledgerTx`) to
 - confirm.
 -
 - Fifthly, log how many tokens were forged.
 -}
